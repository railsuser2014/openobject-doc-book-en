...
To understand how to manage rental products by structuring locations, configure a rental location as
shown in following figure using the menu :menuselection:`Warehouse --> Configuration -->
...
.. figure:: images/stock_rental_location.png
   ...
   *Configuration of a rental location*

Now create a new `Delivery order` that will contain the product which you want to give on rent.
Select the destination location as `Axelor - Rental Location` for this new delivery order.

Using the menu :menuselection:`Warehouse --> Traceability --> Stock Moves` you can find the two `Stock Moves`
for the product for which the `Delivery Order` was created.

* Stock movement from location `Stock` to `Axelor - Rental Location`
* Stock movement from location  `Axelor - Rental Location` to `Stock` (After 30 days)
